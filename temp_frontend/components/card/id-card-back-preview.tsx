...
export function IDCardBackPreview({ data, qrValue, backCardRef, isExportMode = false }: Props) {
  const [composedBg, setComposedBg] = useState<string>(templateBgSrc);
  ...
  useEffect(() => {
    ...
    renderQrOnIdTemplate(value)
      .then((url) => { if (!cancelled) setComposedBg(url); })
      .catch((e) => {
        console.warn('QR render on template failed:', e);
        if (!cancelled) setComposedBg(templateBgSrc);
      });
    return () => { cancelled = true; };
  }, [qrValue]);
  ...
  return (
    <div className="flex justify-center">
      ...
      >
        {/* Composed background - use object-contain in export mode */}
        <img
          src={composedBg}
          alt=""
